...
# 将明文用AES加密
def AES_en(key, data, iv):
    # 将长度不足16字节的字符串补齐
    if len(data) < 16:
        data = pad(data)
    # 创建加密对象
    AES_obj = AES.new(key.encode("utf-8"), AES.MODE_OFB, iv.encode("utf-8"))
    # 完成加密
...
def AES_de(key, data, iv):
    ...
    # 创建解密对象
    AES_de_obj = AES.new(key.encode("utf-8"), AES.MODE_OFB, iv.encode("utf-8"))
    # 完成解密
